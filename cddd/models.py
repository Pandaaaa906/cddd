...
from tensorflow_addons.seq2seq import TrainingSampler, BasicDecoder, dynamic_decode, tile_batch, BeamSearchDecoder
...
class GRUSeq2Seq(BaseModel):
    ...
    def _encoder(self, encoder_emb_inp):
        """Method that defines the encoder part of the translation model graph."""
        encoder_cell = [tf.keras.layers.GRUCell(size) for size in self.cell_size]
    ...
    def _decoder(self, encoded_seq, decoder_emb_inp=None):
        ...
        decoder_cell = [tf.keras.layers.GRUCell(size) for size in self.cell_size]
        decoder_cell = tf.keras.layers.StackedRNNCells(decoder_cell)
        ...
        if self.mode != "DECODE":
            helper = TrainingSampler(time_major=False)
            helper.initialize(
                decoder_emb_inp,
            ...
            outputs, output_state, _ = dynamic_decode(decoder,
                                                      impute_finished=True,
                                                      output_time_major=False)
            return outputs.rnn_output
        else:
            decoder_cell_inital = tile_batch(decoder_cell_inital,
                                             self.beam_width)
            ...
            outputs, output_state, _ = dynamic_decode(
                decoder=decoder,
...
class GRUVAE(GRUSeq2Seq):
    ...
    def _encoder(self, encoder_emb_inp):
        ...
        """Method that defines the encoder part of the translation model graph."""
        encoder_cell = [tf.keras.layers.GRUCell(size) for size in self.cell_size]
        encoder_cell = tf.keras.layers.StackedRNNCells(encoder_cell)
        encoder_outputs, encoder_state = tf.compat.v1.nn.dynamic_rnn(encoder_cell,
                                                                     encoder_emb_inp,
                                                                     sequence_length=self.input_len,
                                                                     dtype=tf.float32,
...
class NoisyGRUSeq2Seq(GRUSeq2Seq):
    ...
    def _encoder(self, encoder_emb_inp):
        ...
        if (self.mode == "TRAIN") & (self.emb_noise > 0.0):
            emb += tf.random.normal(shape=tf.shape(emb),
                                    mean=0.0,
...
class Conv2GRUSeq2Seq(GRUSeq2Seq):
    ...
    def _encoder(self, encoder_emb_inp):
        ...
        emb = tf.compat.v1.layers.dense(tf.reduce_mean(x, axis=1),
                                        self.embedding_size,
                                        activation=self.emb_activation
                                        )
        return emb
